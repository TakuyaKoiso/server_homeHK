...
def on_connect(client, userdata, flag, rc):
  ...
  client.subscribe("BME680_outside")  # subするトピックを設定 
...
def on_message(client, userdata, msg):
  ...
  cursor.execute("insert into BME680_outside values(\'" + str(dt_now.strftime('%Y-%m-%d %H:%M:%S')) + "\', " + str(temperature) + ", " + str(pressure) + ", " + str(pressure_sea) + "," + str(humidity) + ", NULL)")
...
# MQTTの接続設定
client = mqtt.Client("BME680_outside")                 # クラスのインスタンス(実体)の作成
